...
AnsiString, thereby creating a copy of that buffer.
...
Every AnsiString which has to be passed to the application passes the virtual method
"TranslateEncoding" which translates the AnsiString from the current encoding (stored in
...
2000-07-11  HeySt  1.0.7   Removed "Windows" from USES clause
                           ...
                           Added three-exclamation-mark comments for AnsiChar function calls
...
type
  ...
  protected
    ...
    // Returns FBuffer or a pointer to a NUL AnsiChar if Buffer is empty
  ...
  public
    ...
    function LoadExternalEntity(SystemId, PublicId,
      ...
      virtual;
    function TranslateEncoding(const Source: AnsiString): AnsiString;
      virtual;
  ...
  public
    ...
    // Returns next AnsiChar or NIL if EOF is reached. Frees Instance.
  end;
...
const
  ...
  CPartType_Name: array [TPartType] of AnsiString =
    ...
    'Text', 'CDATA');
  CValueType_Name: array [TValueType] of AnsiString =
    ('Normal', 'Implied', 'Fixed', 'Default');
  CAttrDefault_Name: array [TAttrDefault] of AnsiString =
    ('Default', 'Required', 'Implied', 'Fixed');
  CElemType_Name: array [TElemType] of AnsiString =
...
// Convert buffer part to Pascal AnsiString
...
type
  ...
  TXmlPrologEvent   = procedure(Sender: TObject; XmlVersion, Encoding: AnsiString;
    Standalone: boolean) of object;
  TCommentEvent = procedure(Sender: TObject;
    Comment: AnsiString) of object;
  ...
  TErrorEvent = procedure(Sender: TObject;
    ErrorPos: PAnsiChar) of object;
  TExternalEvent = procedure(Sender: TObject;
    SystemId, PublicId, NotationId: AnsiString;
  ...
  protected
    ...
    procedure WhenComment(Comment: AnsiString);
      virtual;
    procedure WhenPI(Target, Content: AnsiString; Attributes: TAttrList);
      virtual;
    procedure WhenDtdRead(RootElementName: AnsiString);
      virtual;
    procedure WhenStartTag(TagName: AnsiString; Attributes: TAttrList);
      virtual;
    procedure WhenEmptyTag(TagName: AnsiString; Attributes: TAttrList);
      virtual;
    procedure WhenEndTag(TagName: AnsiString);
      virtual;
    procedure WhenContent(Content: AnsiString);
      virtual;
    procedure WhenCData(Content: AnsiString);
      virtual;
    ...
    procedure WhenNotation(NotationDef: TNotationDef);
      virtual;
    procedure WhenDtdError(ErrorPos: PAnsiChar);
      virtual;
...
function AnsiToUtf8(Source: ansistring): AnsiString;
  (* Converts the given Windows ANSI (Win1252) AnsiString to UTF-8. *)
var
  I:   integer;  // Loop counter
  U:   word;     // Current Unicode value
  Len: integer;  // Current real length of "Result" AnsiString
begin
...
function Utf8ToAnsi(Source: AnsiString; UnknownChar: AnsiChar = '¿'): ansistring;
          (* Converts the given UTF-8 AnsiString to Windows ANSI (Win-1252).
             If a character can not be converted, the "UnknownChar" is inserted. *)
var
  SourceLen: integer;  // Length of Source AnsiString
  I, K: integer;
...
function DelChars(Source: AnsiString; CharsToDelete: TCharset): AnsiString;
  // Delete all "CharsToDelete" from the AnsiString
var
  I: integer;
begin
  Result := Source;
...
function TrimWs(Source: AnsiString): AnsiString;
  // Trimms off Whitespace characters from both ends of the AnsiString
var
  I: integer;
begin
  // --- Trim Left
...
function StrScanE(const Source: PAnsiChar; const CharToScanFor: AnsiChar): PAnsiChar;
  // If "CharToScanFor" is not found, StrScanE returns the last AnsiChar of the
...
procedure ExtractName(Start: PAnsiChar; Terminators: TCharset; var Final: PAnsiChar);
          (* Extracts the complete Name beginning at "Start".
             ...
             Start:       IN  Pointer to first AnsiChar of name. Is always considered to be valid
             Terminators: IN  Characters which terminate the name
             Final:       OUT Pointer to last AnsiChar of name *)
begin
  Final := Start + 1;
...
procedure ExtractQuote(Start: PAnsiChar; var Content: AnsiString; var Final: PAnsiChar);
          (* Extract a AnsiString which is contained in single or double Quotes.
             Start:    IN   Pointer to opening quote
             Content:  OUT  The quoted AnsiString
...
procedure TXmlParser.AnalyzeDtdc;
          (* Analyze Document Type Declaration
                 ...
                 PI           ::=  '<?' PITarget (S (AnsiChar* - (AnsiChar* '?>' AnsiChar* )))?     '?>'
                 Comment      ::= '<!--' ((AnsiChar - '-') | ('-' (AnsiChar - '-')))*          '-->'  *)
type
...
begin
  ...
  // --- Scan DTDc
  CurFinal := CurStart + 9;    // First AnsiChar after '<!DOCTYPE'
  Phase    := phName;
  repeat
    case CurFinal^ of
      '%':
      ...
      begin
        ...
        begin
          case AttrDef.DefaultType of
            adDefault: ;
            adRequired: ;
            // -!- It is an error in the document if "Attr.Value" is an empty AnsiString
    ...
    begin
      ...
      begin
        if UpCase((CurFinal + 2)^) =
          'X'       // !!! Can't use "AnsiChar" for Unicode characters > 255:
        then
          CurContent := CurContent + AnsiChar(StrToIntDef('$' + Copy(Name, 3, MaxInt), 32))
        else
          CurContent := CurContent + AnsiChar(StrToIntDef(Copy(Name, 2, MaxInt), 32));
        CurFinal := P + 1;
        EXIT;
      end
            ...
            begin
              ...
              begin
                ...
                if F <>
                  ...
                  AttrDef.TypeDef := AnsiString(Final + 1);
                AttrDef.TypeDef :=
...
var
  ...
  DER:   TDtdElementRec;
begin
  Final     := Start + 8;   // First AnsiChar after <!ENTITY
  DER.Start := Start;
  Phase     := phName;
  IsParamEntity := False;
...
begin
  ...
  repeat
    ...
    if CompareText(Str[PosAmp + 2], 'x') =
      0          // !!! Can't use "AnsiChar" for Unicode characters > 255
    then
      Str[PosAmp] := AnsiChar(StrToIntDef('$' + Copy(Str, PosAmp + 3, Len - 4), 0))
    else
      Str[PosAmp] := AnsiChar(StrToIntDef(Copy(Str, PosAmp + 2, Len - 3), 32));
...
// Recursively replaces all Parameter Entity References in the AnsiString
...
// Recursively replaces General Entity References in the AnsiString
  procedure ReplaceEntities(var Str: AnsiString);
  var
    Start:   integer;
    PAmp:    PAnsiChar;
...
function TScannerXmlParser.LoadExternalEntity(SystemId, PublicId, Notation: AnsiString)
: TXmlParser;
begin

...
procedure DrawImitGPS(AsphCanvas: TAsphyreCanvas; Color, Color2:Cardinal;
    Smooth: Boolean; AnimPos: Double; Arrows: Boolean);
...
begin
   ...
   SetLength(ReadyPoints, RouteCount);
...
procedure DrawImitGPS(AsphCanvas: TAsphyreCanvas; Color, Color2:Cardinal;
  ...
  begin
     try
       MinL := 350 * _Scale/Scale;
       MinLineL := 350 * _Scale/Scale;
     except
       MinL := 350;
       MinLineL := 350;
     end;

    if Length(GPS) <= 1 then
      exit;

    L := 0;
    for I := 1 to Length(GPS)-1 do
    BEGIN
        P1 := MapToScreen(GPS[i-1].X, GPS[i-1].Y);
        P2 := MapToScreen(GPS[i].X, GPS[i].Y);


        Ang := arctan2(GPS[i].X - GPS[i-1].X,
                       GPS[i].Y - GPS[i-1].Y);
        ...
        begin
           ...
           AsphCanvas.TexMap(pRotate4c(Point2(P1.x,P1.y),Point2(16, 16), Ang+fi),cColor4(Color));
        end;

        if (I = Length(GPS)-1) and (OnScreen)  then
        begin
           AsphCanvas.UseImagePx(AsphImages.Image['arrow1.image'], pxBounds4(0, 0, 32, 32));
           AsphCanvas.TexMap(pRotate4c(Point2(P2.x,P2.y),Point2(16, 16), Ang+fi),cColor4(Color));
        end;
        ...
        begin
          ...
          begin
             AsphCanvas.UseImagePx(AsphImages.Image['arrow1.image'], pxBounds4(0, 0, 32, 32));
             AsphCanvas.TexMap(pRotate4c(Point2((P2.x + P1.x)/2,(P1.y + P2.y)/2),Point2(16, 16), Ang+fi),cColor4(Color));
          end;
        end;

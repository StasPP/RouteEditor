...
type
  ...
  type
  TKnot = record
    Name    :String;   // им€ профил€
    L       :Integer;  // номер расстановки
    NameKind:Byte;     // тип имени
    NameKind2:Byte;    // раздел€ть/сливать номера
    ...
    StepOut :Integer;      // вынос через шаг или є шаблона
      ...
      Begin
          ...
          begin
            ...
            if Yimax <> Yimin then
              ...
              begin
                 // второй раз, дл€ соблюдени€ четности
                 Xs[XsCount] := Ximin + c*(Ximax - Ximin);
                 inc(XsCount);
              end;
...
procedure LoadKMLFile(FileName:String; doRoutes, doPoints, doArea,
  ...
  function FindTaggedData(var StartI, StartJ, EndI, EndJ: Integer;
                          /// строка и символ — и ѕќ внутри тегов
  ...
  function GetKind(var SI, SJ, EI, EJ: Integer):integer;   // ќпределить тип
  var I, J :Integer;
  ...
  /// на входе и выходе - строка и символ — и ѕќ + тип объекта
  ...
  begin
     ...
     begin
       ...
       Kind := GetKind(SI, SJ, EI, EJ);     // ќпределить тип
       if Kind > 0 then
       begin
          GetObjData(StartI, StartJ, EndI, EndJ, Kind); // «агрузка данных заданного дл€ типа
          result := Kind;
       end;
     end
     else
...
begin
 ...
 for I := 0 to FrameCount - 1 do
  ...
  repeat
    ...
    begin
       if abs(Xs[i*2] - Xs[i*2+1]) < 0.01 then   /// Ќулева€ длина
       begin
          if i = XsCount div 2 -1 then
            if k = 1 then   /// Ќи одного не добавлено
              dec(RowI);
...
begin
 ...
 try
  ...
  Begin
      ...
      begin                            {попал на отрезок + мин. дистанци€}
         if J < length(Route[I].WPT) - 1 then
...
Begin
   ...
   for j := 0 to Length(SelectedMarkers) - 1 do  {ѕровер€ю только выделенные}
   Begin
     I := SelectedMarkers[j];
     P := MapToScreen(Markers[i].x, Markers[i].y);
...
Begin
   ...
   for j := 0 to Length(SelectedKnots) - 1 do  {ѕровер€ю только выделенные}
   Begin
     I := SelectedKnots[j];
     P := MapToScreen(KnotPoints[i].Cx, KnotPoints[i].Cy);
     Result := (abs(P.x-CanvCursor.x) <= Sens) and (abs(P.y - CanvCursor.y) <= Sens);

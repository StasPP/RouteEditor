...
begin
  UpBtn.Enabled    := FirstSel(RouteMarks) > 0;
  UpBtn2.Enabled   := FirstSel(RouteMarks) > 0;

  DownBtn.Enabled  := (LastSel(RouteMarks) >= 0) and
      (LastSel(RouteMarks) < RouteMarks.Items.Count -1);
  DownBtn2.Enabled := (LastSel(RouteMarks) >= 0) and
      (LastSel(RouteMarks)< RouteMarks.Items.Count -1);
...
begin
  ...
  for I := 0 to Length(_Selected) - 1 do
     RouteMarks.Selected[_Selected[I]] := True;
...
{  I := RouteMarks.ItemIndex;
  ...
  while _Selected[Length(_Selected) - 1] < RouteMarks.Count-1 do
  for I := Length(_Selected) - 1 downto 0 do
  begin
    J := B[_Selected[I]+1];
...
{  I := RouteMarks.ItemIndex;
  ...
  if _Selected[Length(_Selected) - 1] < RouteMarks.Count-1 then
  for I := Length(_Selected) - 1 downto 0 do
  begin
    J := B[_Selected[I]+1];

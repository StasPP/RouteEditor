...
var
  ...
  BrkStep, SpinEdit3, MinL, NewStep :TFloatSpinEdit;
implementation
     ...
     1:  case NameLabel.Checked of
             true:
                 MakeMNKRoute(MainTrack, T1, T2, RMethod.ItemIndex, MinL.Value,
                      DoMinL.Checked, isReset.Checked, NameEdit.Text, NS);
             false:
                 MakeMNKRoute(MainTrack, T1, T2, RMethod.ItemIndex, MinL.Value,
                      DoMinL.Checked, isReset.Checked, '', NS);
...
begin
 ...
 NewStep  := TFloatSpinEdit.Create(Frops);
 ...
 NewStep.Parent  := NewStep_.Parent;
 NewStep.OnClick := NewStep_.OnClick;
 NewStep.OnChange := NewStep_.OnChange;
 NewStep.Left := NewStep_.Left;
 NewStep.Top := NewStep_.Top;
 NewStep.Width := NewStep_.Width;
 NewStep.Height := NewStep_.Height;
 NewStep.Value := NewStep_.Value;
 NewStep.MinValue := NewStep_.MinValue;
 NewStep.MaxValue := NewStep_.MaxValue;
 NewStep.Increment := NewStep_.Increment;
 NewStep_.Hide;
...
begin
  BrkStep.Destroy; SpinEdit3.Destroy;  MinL.Destroy;   NewStep.Destroy;
end;

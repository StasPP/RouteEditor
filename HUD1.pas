...
var
  ...
  NordDirectionXY:TPoint;
  NordDirectionR:Integer;
implementation
...
procedure ScaleLine(AsphCanvas:TAsphyreCanvas;
   ...
   begin
      ...
      if (Scale >=100) or
         ((Scale >= 10) and (abs(Scale/10 - trunc(Scale/10)) < 0.1)) then
          s := IntTostr(Trunc(Scale)div 10) + KMsign
      else if Scale >=10  then
          s := Format('%n',[Scale/10]) + KMsign
      else if (Scale >= 0.1)or
         ((Scale >= 0.01) and (abs(Scale*100 - round(Scale*100)) <= 0.01)) then
...
var X1, X2, Y1, Y2, Yi, Yi2, step, lastY, sW :integer;
  ...
  begin
    ...
    BEGIN
      ...
      AsphCanvas.Line(DispSize.X - 23, trunc(iY), DispSize.X - 17, trunc(iY), FntColor);
  ...
  if TrackColorSch.min = TrackColorSch.max then
   ...
   BEGIN
      ...
      begin
        ...
        if (TrackColorSch.isRound) and (N < TrackColorSch.CSMarkCount - 2) and (s1 <> s2) then
          s2 := MarkSt(TrackColorSch.CSMarks[N+1].Mark-1);
...
//  AsphCanvas.Antialias := true;
  ...
  NordDirectionXY := Point( DispSize.X - 40 , 40 );
  NordDirectionR  := 36;

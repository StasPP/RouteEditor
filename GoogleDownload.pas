...
interface
 ...
 function GetYandexStaticMapURL( B,L: double; MapStyle, YaKey, YaLang :string;
                                    ZoomLevel: integer):string;
 procedure _GetYandexStaticMap(URL:String; YStyle: string; var Image: TImage);
...
begin
   Result:=False;
   sAppName := ExtractFileName({Application.ExeName}'RouteEditor');
...
begin
  ...
  try
    ...
    // Idhttp.Get(Url, StreamData); //Send the request and get the image
     GetInetFile(URL, Fname);
     //StreamData.Seek(0,soFromBeginning);
     if Pos('sat',YStyle) <> 0 then
     begin
       JPEGImage.LoadFromFile(Fname);//load the image in a Stream
...
//dHTTP.IOHandler := IdSSL;
 IdHTTP.AllowCookies := True;
 IdHTTP.HandleRedirects := True;
// IdHTTP.Request.CustomHeaders.Add('X-Forwarded-For', Format('%d.%d.%d.%d', [Random(255), Random(255), Random(255), Random(255)]));
// IdHTTP.Request.UserAgent := Format('Mozilla/%d.0 (Windows NT %d.%d; rv:2.0.1) Gecko/20100101 Firefox/%d.%d.%d', [RandomRange(3, 5), RandomRange(3, 5), Random(2), RandomRange(3, 5), Random(5), Random(5)]);

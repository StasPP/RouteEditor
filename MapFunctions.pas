...
uses Windows, GeoFunctions, GeoClasses, TabFunctions, Math, PointClasses;
...
type
  ...
  TRoutePosAndDist = record
      Pos, Dist, DistToV0, DistTo0, x, y :Double;
      seg: Integer;
  end;
  ...
  function PointInRect(x1, y1, x2, y2, x, y: Double): boolean;
  ...
  function PointXY(x,y :Double): TMyPoint;
  const
  ...
  VshiftY   :Integer = 0;
  ClickMode :integer = 1;
  RulerLoops:Boolean = False;
  MKeyShift :TMyPoint;
  MShift, CanvCursor :TPoint;
  CanvCursorBL       :TLatLong;

  SpecialScale :Boolean = false;
  /// Projection
  UTM          :Boolean = true;
  South        :Boolean = false;
  MyZone       :Integer = 14;
